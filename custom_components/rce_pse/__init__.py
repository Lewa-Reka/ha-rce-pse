...
async def async_migrate_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    if config_entry.version < 2:
        data = migrate_option_defaults(
            migrate_price_unit_in_mapping(migrate_legacy_time_values(config_entry.data))
        )
        options = migrate_option_defaults(
            migrate_price_unit_in_mapping(
                migrate_legacy_time_values(config_entry.options)
    ...
    if config_entry.version < 3:
        data = migrate_option_defaults(config_entry.data)
        options = migrate_option_defaults(config_entry.options if config_entry.options else {})
        hass.config_entries.async_update_entry(
            config_entry,
            data=data,
            options=options,
