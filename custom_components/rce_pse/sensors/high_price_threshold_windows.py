...
class RCEHighPriceThresholdWindowSensor(RCEBaseSensor):
...
class RCETodayHighPriceThresholdWindowStartSensor(RCEHighPriceThresholdWindowSensor):

    def __init__(self, coordinator: RCEPSEDataUpdateCoordinator, config_entry: ConfigEntry) -> None:
        super().__init__(coordinator, config_entry, "today_high_price_threshold_window_start")
    ...
    def native_value(self) -> datetime | None:
        ...
        window = self.calculator.find_first_window_above_threshold(today_data, threshold)
        if not window:
            return None
        try:
...
class RCETodayHighPriceThresholdWindowEndSensor(RCEHighPriceThresholdWindowSensor):

    def __init__(self, coordinator: RCEPSEDataUpdateCoordinator, config_entry: ConfigEntry) -> None:
        super().__init__(coordinator, config_entry, "today_high_price_threshold_window_end")
        self._attr_device_class = SensorDeviceClass.TIMESTAMP
...
class RCETomorrowHighPriceThresholdWindowStartSensor(RCEHighPriceThresholdWindowSensor):

    def __init__(self, coordinator: RCEPSEDataUpdateCoordinator, config_entry: ConfigEntry) -> None:
        super().__init__(coordinator, config_entry, "tomorrow_high_price_threshold_window_start")
        self._attr_device_class = SensorDeviceClass.TIMESTAMP
        self._attr_icon = "mdi:clock-start"
    ...
    def native_value(self) -> datetime | None:
        ...
        if not tomorrow_data:
            return None
        threshold = self.get_config_value(CONF_HIGH_PRICE_THRESHOLD, DEFAULT_HIGH_PRICE_THRESHOLD)
        window = self.calculator.find_first_window_above_threshold(tomorrow_data, threshold)
        if not window:
            return None
        try:
            start_time_str = window[0]["period"].split(" - ")[0]
...
class RCETomorrowHighPriceThresholdWindowEndSensor(RCEHighPriceThresholdWindowSensor):

    def __init__(self, coordinator: RCEPSEDataUpdateCoordinator, config_entry: ConfigEntry) -> None:
        super().__init__(coordinator, config_entry, "tomorrow_high_price_threshold_window_end")
        self._attr_device_class = SensorDeviceClass.TIMESTAMP
        self._attr_icon = "mdi:clock-end"
